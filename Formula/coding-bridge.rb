class CodingBridge < Formula
  ...
  url "https://files.pythonhosted.org/packages/60/f4/f44e60257d7e6c36b004f6595c3eaddbf93567de663c0e52a3910e6a78bd/coding_bridge-2026.7.28.3.tar.gz"
  sha256 "4ba8d5b2d0333f765bac0e3d8e12f3d8cec4eb923095d4a169e5222c8bfaca88"
  license "AGPL-3.0-or-later"

  # cryptography, pydantic-core and rpds-py ship Rust/native extensions that
  # compile from sdist at install time.
  depends_on "pkg-config" => :build
  depends_on "rust" => :build
  depends_on "openssl@3"
  depends_on "python@3.14"

  # resource blocks below are generated by `brew update-python-resources`.
  # Keep the python@ pin in sync with the interpreter used to generate them.
  ...
  def install
    # cryptography's Rust build needs to find Homebrew's OpenSSL.
    ENV["OPENSSL_DIR"] = formula_opt_prefix("openssl@3")
    virtualenv_install_with_resources
  end

  # Run the daemon as a user LaunchAgent (macOS) / systemd unit (Linux):
  #   coding-bridge pair            # once, interactively (REQUIRED first)
  #   brew services start coding-bridge
  # Pair BEFORE starting: an unpaired `run` exits non-zero, so starting the
  # service first would just restart-throttle until you pair. It runs as your
  # user, so it keeps your Claude/Codex login.
  service do
    run [opt_bin/"coding-bridge", "run"]
    # Restart on crash/network drop, but not after a clean stop.
    keep_alive crashed: true
    log_path var/"log/coding-bridge.log"
    error_log_path var/"log/coding-bridge.log"
  end

  test do
    # `status` needs no network or pairing; it prints config + pairing state.
    assert_match "Paired", shell_output("#{bin}/coding-bridge status")
  end
end
